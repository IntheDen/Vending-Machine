...
public class VendingMachine {
    ...
    public String getCurrentTime() {
        ...
        int itemPrice = Integer.parseInt(menu.getItemSelected().getPrice()) * 100;
        int newBalance = vendingMachineCoinBox.getBalance() * 100;        //MONEY REMAINING (FROM BEFORE) + MONEY INSERTED
        int itemQuantity = menu.getItemSelected().getQuantity();     //QUANTITY OF THE ITEM
        String itemName = menu.getItemSelected().getName();          //ITEM NAME
        ...
        if (itemPrice > newBalance) {
            ...
            System.out.println(menu.getItemSelected().getMessage());
            int currItemQuantity = itemQuantity - 1;                      //UPDATING ITEM QUANTITY
            menu.getItemSelected().setQuantity(currItemQuantity);    //SETTING ITEM QUANTITY TO THE UPDATE
